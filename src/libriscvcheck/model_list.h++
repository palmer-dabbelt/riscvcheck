...
#ifndef LIBRISCVCHECK__MODEL_LIST_HXX
#define LIBRISCVCHECK__MODEL_LIST_HXX
...
namespace libriscvcheck {
    ...
    class model_list {
    private:
        static std::map<std::string, std::function<model::ptr(void)>> _models;

    public:
        /* Searches the model database for a model with the given
         * name. */
        static model::ptr lookup(const std::string& name);

        /* Inserts a model constructor into the database. */
